...
def gauss1D_cart(x: np.ndarray,
    ...
    norm_sum : bool, optional
        whether to normalize by the sum instead of the peak (the default is True)
    ...
    g1 = stats.norm.pdf(x, loc=mu, scale=sigma)
    if norm_sum:
        return g1 / np.sum(g1)
    else:
        return g1 / np.max(g1)
...
def vonMises1D(x: np.ndarray,
    ...
    """
    vm = stats.vonmises.pdf(x, kappa=kappa, loc=mu)
    if norm_sum:
        return vm / np.sum(vm)
    else:
        return vm / np.max(vm)
...
def gauss2D_iso_cart(x: np.ndarray,
    ...
    """
    ic = np.exp(-((x-mu[0])**2 + (y-mu[1])**2)/(2*sigma**2))
    if norm_sum:
        return ic / np.sum(ic)
    else:
        return ic / np.max(ic)
...
def gauss2D_logpolar(ecc: np.ndarray,
    ...
    if norm_sum:
        return logpolar_Z / np.sum(logpolar_Z)
    else:
